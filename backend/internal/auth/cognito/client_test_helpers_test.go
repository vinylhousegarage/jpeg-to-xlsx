...
func newTestOAuthServer(
	...
	oauthServer.server = httptest.NewServer(
		http.HandlerFunc(
			func(
				...
				oauthServer.tokenRequestForm =
					cloneURLValues(
						request.Form,
					)
				oauthServer.
					tokenRequestHasBasicAuth =
					hasBasicAuth
				oauthServer.tokenRequestClientID =
					clientID
				oauthServer.
				...
				status :=
					oauthServer.tokenResponseStatus
				body :=
					oauthServer.tokenResponseBody
