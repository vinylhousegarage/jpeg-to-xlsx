...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// JSONで返却
	h.writeDownloadResponse(w, url, expiresAt)
}

// 検証メソッド
func (h *Handler) validateRequest(r *http.Request) (*storage.GetPresignRequest, error) {
	if r.Method != http.MethodPost {
		return nil, apierror.New(apierror.ErrorCodeInvalidMethod, http.StatusMethodNotAllowed, nil)
	}

	var req storage.GetPresignRequest
...
// JSONレスポンス書き込みメソッド
func (h *Handler) writeDownloadResponse(w http.ResponseWriter, url string, expiresAt time.Time) {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(http.StatusOK)

	resp := storage.GetPresignResponse{
		ExpiresAt:   expiresAt,
		DownloadURL: url,
	}
