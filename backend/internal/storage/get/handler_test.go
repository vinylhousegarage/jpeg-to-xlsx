...
func (m *mockPresignerForHandler) PresignGetObject(
	_ context.Context,
...
func TestHandler_ServeHTTP(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:                  "Error: returns method not allowed for PUT request",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req := httptest.NewRequest(tt.method, "/presign/get", bodyReader)
			...
			if mockPresigner.called != tt.expectPresignerCalled {
				t.Errorf(
					"PresignGetObject() called = %t, want %t",
			...
			if tt.expectedObjectKey != "" {
				if mockPresigner.gotInput == nil {
					t.Fatal("PresignGetObject() input is nil")
				}

				gotObjectKey := aws.ToString(mockPresigner.gotInput.Key)

				if gotObjectKey != tt.expectedObjectKey {
					t.Errorf("object key = %q, want %q", gotObjectKey, tt.expectedObjectKey)
				}
			}
		})
	}
